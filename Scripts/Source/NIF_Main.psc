...
int property morphModInfluencePercentages Auto
; JMap(morphModInfluence) declared above
; This is the JMap that contains all morphID keys with an array of Mod Booleans that affect it.AddNodeOverrideBool
...
Event onInit()
    ...
    RegisterForModEvent("NIF_MorphValueRequestReceive", "MorphValueRequestReceive")
EndEvent
...
function loadJContainerReferences()
    ...
    if morphModInfluencePercentages == 0
        morphModInfluencePercentages = JMap_object()
        JValue_retain(morphModInfluencePercentages)
        NimorphusLog(Debug_Info, morphModInfluencePercentages + " morphModInfluencePercentages")
    endif
endfunction
...
float Function ApplyMorphValueAdjustments(Actor kActor, String morphID, String modName, int requestType = 0)
    ...
    ; 3. Apply mod specific influences
    
    ; 4. Apply morph range adjustment
...
int Function GetOrCreateModInfluencePercentages(String morphID)
    int modPercentMap = 0
    if JMap_hasKey(morphModInfluencePercentages, morphID)
        modPercentMap = JMap_getObj(morphModInfluencePercentages, morphID)
    else
        modPercentMap = JMap_object()
        JValue_retain(modPercentMap)
        JMap_setObj(morphModInfluencePercentages, morphID, modPercentMap)
    endif
    return modPercentMap
EndFunction
...
; ========== SKSE Events Retreival Line ==========
; Response of -1500 = Actor is None
; Response of -1600 = Morph ID is not found for actor
; Response of -1700 = Mod not found for this MorphID
Function MorphValueRequestReceive(Actor kActor, String morphID, String modKey)
    ; Check if the actor exists in the actorMorphData map
    if !kActor
        NimorphusLog(Debug_Warn, "Error: Actor is None")
        MorphValueRequestSend(modKey, 0.0)
        return
    endif
    Form actorFormID = kActor as Form
    ; Check if the actor data exists in the map
    if !JMap_hasKey(mapActorData, actorFormID)
        NimorphusLog(Debug_Warn, "Actor not found in mapActorData.")
        MorphValueRequestSend(modKey, -1500.0)
        return
    endif
    ; Retrieve the actor's morph data map
    int actorMorphMap = JMap_getObj(mapActorData, actorFormID)    
    ; Check if the actor's morph map contains the requested morphID
    if !JMap_hasKey(actorMorphMap, morphID)
        NimorphusLog(Debug_Warn, "Morph ID not found for this actor.")
        MorphValueRequestSend(modKey, -1600.0)
        return
    endif
    ; Retrieve the morph-specific map (modKey -> morphValue map)
    int morphModMap = JMap_getObj(actorMorphMap, morphID)
    ; Check if the modKey exists in this morph's map
    if !JMap_hasKey(morphModMap, modKey)
        NimorphusLog(Debug_Warn, "Mod key not found for this morph ID.")
        MorphValueRequestSend(modKey, -1700.0)
        return
    endif
    ; Retrieve the morph value for this modKey
    float morphValue = JMap_getFlt(morphModMap, modKey)    
    NimorphusLog(Debug_Info, "MorphValue for Actor " + actorFormID + " | MorphID " + morphID + " | Mod " + modKey + " = " + morphValue)    
    MorphValueRequestSend(modKey, morphValue)
EndFunction
; Function to send morph value from MorphValueRequest
Function MorphValueRequestSend(string modName, float value)
	int morphValueSend = ModEvent.Create("NIF_MorphValueRequestSend")	
	If (morphValueSend)
        ModEvent.PushString(morphValueSend, modName)
		ModEvent.PushFloat(morphValueSend, value)
		ModEvent.Send(morphValueSend)
	EndIf
EndFunction

function stuff()
    float value = Nimorphus_Global.RequestCurrentMorphValue()
endfunction
